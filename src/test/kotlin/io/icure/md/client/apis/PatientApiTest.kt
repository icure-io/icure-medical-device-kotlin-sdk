...
class PatientApiTest() {
	...
	fun createOrModifyPatientTest(fileName: String) = runBlocking {

        if (TestUtils.skipEndpoint(fileName, "createOrModifyPatient")) {
            assertTrue(true, "Test of createOrModifyPatient endpoint has been skipped")
        } else {
            try{
                createForModification(fileName)
                val credentialsFile = TestUtils.getCredentialsFile(fileName, "createOrModifyPatient")
                val patient: Patient = TestUtils.getParameter<Patient>(fileName, "createOrModifyPatient.patient")!!.let {
                    (it as? PatientDto)?.takeIf { TestUtils.isAutoRev(fileName, "createOrModifyPatient") }?.let {
                ...
                val response = api(credentialsFile).createOrModifyPatient(patient = patient)

                val testFileName = "PatientApi.createOrModifyPatient"
                val file = File(workingFolder + File.separator + this::class.simpleName + File.separator + fileName, "$testFileName.json")
                try {
                    val objectFromFile = (response as? Flow<ByteBuffer>)?.let { file.readAsFlow() } ?: objectMapper.readValue(file,  if (response as? List<Patient>? != null) {
                        if ("Patient".contains("String>")) {
                            object : TypeReference<List<String>>() {}
                        } else {
                            object : TypeReference<List<Patient>>() {}
                        }
                    } else if(response as? kotlin.collections.Map<String, String>? != null){
                        object : TypeReference<Map<String,String>>() {}
                    } else {
                        object : TypeReference<Patient>() {}
                    })
                    assertAreEquals("createOrModifyPatient", objectFromFile, response)
	...
	fun deletePatientTest(fileName: String) = runBlocking {
        ...
        if (TestUtils.skipEndpoint(fileName, "deletePatient")) {
            assertTrue(true, "Test of deletePatient endpoint has been skipped")
        } else {
            try{
                createForModification(fileName)
                val credentialsFile = TestUtils.getCredentialsFile(fileName, "deletePatient")
                val id: kotlin.String = TestUtils.getParameter<kotlin.String>(fileName, "deletePatient.id")!!.let {
                    (it as? PatientDto)?.takeIf { TestUtils.isAutoRev(fileName, "deletePatient") }?.let {
                    val id = it::class.memberProperties.first { it.name == "id" }
                    val currentRev = api(credentialsFile).getPatient(id.getter.call(it) as String).rev
                    it.copy(rev = currentRev)
                    } as? kotlin.String ?: it
                ...
                val response = api(credentialsFile).deletePatient(id = id)
    ...
    @ParameterizedTest
    @MethodSource("fileNames") // six numbers
	fun filterPatientsTest(fileName: String) = runBlocking {

        if (TestUtils.skipEndpoint(fileName, "filterPatients")) {
            assertTrue(true, "Test of filterPatients endpoint has been skipped")
        } else {
            try{
                createForModification(fileName)
                val credentialsFile = TestUtils.getCredentialsFile(fileName, "filterPatients")
                val filter: Filter = TestUtils.getParameter<Filter>(fileName, "filterPatients.filter")!!.let {
                    (it as? PatientDto)?.takeIf { TestUtils.isAutoRev(fileName, "filterPatients") }?.let {
                    val id = it::class.memberProperties.first { it.name == "id" }
                    val currentRev = api(credentialsFile).getPatient(id.getter.call(it) as String).rev
                    it.copy(rev = currentRev)
                    } as? Filter ?: it
                ...
                val response = api(credentialsFile).filterPatients(filter = filter)

                val testFileName = "PatientApi.filterPatients"
                val file = File(workingFolder + File.separator + this::class.simpleName + File.separator + fileName, "$testFileName.json")
                try {
                    val objectFromFile = (response as? Flow<ByteBuffer>)?.let { file.readAsFlow() } ?: objectMapper.readValue(file,  if (response as? List<PaginatedListPatient>? != null) {
                        if ("PaginatedListPatient".contains("String>")) {
                            object : TypeReference<List<String>>() {}
                        } else {
                            object : TypeReference<List<PaginatedListPatient>>() {}
                        }
                    } else if(response as? kotlin.collections.Map<String, String>? != null){
                        object : TypeReference<Map<String,String>>() {}
                    } else {
                        object : TypeReference<PaginatedListPatient>() {}
                    })
                    assertAreEquals("filterPatients", objectFromFile, response)
                    println("Comparison successful")
                }
                catch (e: Exception) {
                    when (e) {
	...
	fun getPatientTest(fileName: String) = runBlocking {
        ...
        if (TestUtils.skipEndpoint(fileName, "getPatient")) {
            ...
            try{
                ...
                val id: kotlin.String = TestUtils.getParameter<kotlin.String>(fileName, "getPatient.id")!!.let {
                ...
                val response = api(credentialsFile).getPatient(id = id)
    ...
    @ParameterizedTest
    @MethodSource("fileNames") // six numbers
	fun matchPatientsTest(fileName: String) = runBlocking {

        if (TestUtils.skipEndpoint(fileName, "matchPatients")) {
            assertTrue(true, "Test of matchPatients endpoint has been skipped")
        } else {
            try{
                createForModification(fileName)
                val credentialsFile = TestUtils.getCredentialsFile(fileName, "matchPatients")
                val filter: Filter = TestUtils.getParameter<Filter>(fileName, "matchPatients.filter")!!.let {
                    (it as? PatientDto)?.takeIf { TestUtils.isAutoRev(fileName, "matchPatients") }?.let {
                    val id = it::class.memberProperties.first { it.name == "id" }
                    val currentRev = api(credentialsFile).getPatient(id.getter.call(it) as String).rev
                    it.copy(rev = currentRev)
                    } as? Filter ?: it
                ...
                val response = api(credentialsFile).matchPatients(filter = filter)

                val testFileName = "PatientApi.matchPatients"
                val file = File(workingFolder + File.separator + this::class.simpleName + File.separator + fileName, "$testFileName.json")
                try {
                    val objectFromFile = (response as? Flow<ByteBuffer>)?.let { file.readAsFlow() } ?: objectMapper.readValue(file,  if (response as? List<kotlin.String>? != null) {
                    ...
                    assertAreEquals("matchPatients", objectFromFile, response)
                    println("Comparison successful")
                }
                catch (e: Exception) {
                    when (e) {
